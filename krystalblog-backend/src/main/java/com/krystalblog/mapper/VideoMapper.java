...
public interface VideoMapper extends BaseMapper<Video> {
    ...
    @Update("UPDATE videos SET views = views + 1 WHERE id = #{id}")
    void incrementViews(@Param("id") Long id);

    @Update("UPDATE videos SET likes_count = likes_count + #{delta} WHERE id = #{id}")
    void updateLikesCount(@Param("id") Long id, @Param("delta") int delta);

    @Update("UPDATE videos SET comments_count = comments_count + #{delta} WHERE id = #{id}")
